...
do_format()
{
    ...
    if [ "$PARTTABLE" = "gpt" ]; then
        ...
        parted -s ${DEVICE} mkpart primary ext4 50MB 6144MB
    else
        BOOT_NUM=
        STATE_NUM=1
        parted -s ${DEVICE} mkpart primary ext4 0% 6094MB
    fi
...
do_copy()
{
    ...
    # import images
    ctr images import /var/lib/rancher/k3s/agent/images/harvester*
